...
import { YamlException } from "@/index";

describe("YamlException", () => {
  describe("constructor", () => {
    test("should create an instance with message only", () => {
      const exception = new YamlException("File not found", "NOT_FOUND");
      expect(exception).toBeInstanceOf(YamlException);
      expect(exception).toBeInstanceOf(Error);
      expect(exception.key).toBe("NOT_FOUND");
    });

    test("should create an instance with message and data", () => {
      const data = { path: "/tmp/file.yml", operation: "read" };
      const exception = new YamlException("Failed to read file", "READ_FAILED", data);
      expect(exception).toBeInstanceOf(YamlException);
  ...
  describe("name", () => {
    test("should have name set to YamlException", () => {
      const exception = new YamlException("Test error", "TEST_ERROR");
      expect(exception.name).toBe("YamlException");
    });
  });

  describe("message", () => {
    test("should have correct message", () => {
      const exception = new YamlException("Custom error message", "CUSTOM_ERROR");
      expect(exception.message).toBe("Custom error message");
    });

    test("should handle empty message", () => {
      const exception = new YamlException("", "EMPTY_MESSAGE");
  ...
  describe("status", () => {
    test("should have default status 500 (Internal Server Error)", () => {
      const exception = new YamlException("Test", "TEST_STATUS");
  ...
  describe("data", () => {
    test("should have empty data by default", () => {
      const exception = new YamlException("Test", "TEST_DATA");
      expect(exception.data).toEqual({});
    });

    test("should store provided data", () => {
      const data = { path: "/tmp/file.yml", error: "ENOENT" };
      const exception = new YamlException("Test", "TEST_DATA", data);
    ...
    test("should not allow data modification", () => {
      const exception = new YamlException("Test", "TEST_DATA", { key: "value" });
      expect(() => {
  ...
  describe("date", () => {
    test("should have a date property", () => {
      ...
      const exception = new YamlException("Test", "TEST_DATE");
  ...
  describe("stack", () => {
    test("should have a stack trace", () => {
      const exception = new YamlException("Test", "TEST_STACK");
  ...
  describe("stackToJson", () => {
    test("should convert stack to JSON array", () => {
      const exception = new YamlException("Test", "TEST_STACK_JSON");
    ...
    test("should have source property in each frame", () => {
      const exception = new YamlException("Test", "TEST_STACK_SOURCE");
      const stackJson = exception.stackToJson();
  ...
  describe("inheritance", () => {
    test("should be throwable", () => {
      expect(() => {
        throw new YamlException("Test error", "TEST_THROWABLE");
      }).toThrow(YamlException);
    });

    test("should be catchable as Error", () => {
      try {
        throw new YamlException("Test error", "TEST_CATCHABLE");
      } catch (error) {
    ...
    test("should work with try-catch pattern", () => {
      let caught = false;
      try {
        throw new YamlException("Operation failed", "TEST_TRY_CATCH", { path: "/test" });
      } catch (error) {
