...
describe("buildFilter", () => {
  describe("comparison operators", () => {
    test("= with string value", () => {
      const filter: FilterType<TestData> = { field: "category", op: "=", value: "books" };
      expect(buildFilter(filter)).toBe("metadata.category = 'books'");
    });

    test("= with number value", () => {
      const filter: FilterType<TestData> = { field: "score", op: "=", value: 42 };
      expect(buildFilter(filter)).toBe("metadata.score = 42");
    });

    test("> operator", () => {
      const filter: FilterType<TestData> = { field: "score", op: ">", value: 10 };
    ...
    test("< operator", () => {
      const filter: FilterType<TestData> = { field: "score", op: "<", value: 100 };
    ...
    test("should preserve unsupported operators for forward compatibility", () => {
      const filter = { field: "score", op: "!=", value: 42 } as unknown as FilterType<TestData>;
      expect(buildFilter(filter)).toBe("metadata.score != 42");
    });
  });
  ...
  describe("null checks", () => {
    test("IS NULL", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS NULL" };
      expect(buildFilter(filter)).toBe("metadata.category IS NULL");
    });

    test("IS NOT NULL", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS NOT NULL" };
  ...
  describe("boolean checks", () => {
    test("IS TRUE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS TRUE" };
      expect(buildFilter(filter)).toBe("metadata.category IS TRUE");
    });

    test("IS NOT TRUE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS NOT TRUE" };
      expect(buildFilter(filter)).toBe("metadata.category IS NOT TRUE");
    });

    test("IS FALSE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS FALSE" };
      expect(buildFilter(filter)).toBe("metadata.category IS FALSE");
    });

    test("IS NOT FALSE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IS NOT FALSE" };
  ...
  describe("IN operator", () => {
    test("IN with string values", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IN", value: ["books", "movies"] };
      expect(buildFilter(filter)).toBe("metadata.category IN ('books', 'movies')");
    });

    test("IN with number values", () => {
      const filter: FilterType<TestData> = { field: "score", op: "IN", value: [1, 2, 3] };
      expect(buildFilter(filter)).toBe("metadata.score IN (1, 2, 3)");
    });

    test("IN with mixed values", () => {
      const filter: FilterType<TestData> = { field: "category", op: "IN", value: ["books", 42] };
  ...
  describe("LIKE operators", () => {
    test("LIKE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "LIKE", value: "%book%" };
      expect(buildFilter(filter)).toBe("metadata.category LIKE '%book%'");
    });

    test("NOT LIKE", () => {
      const filter: FilterType<TestData> = { field: "category", op: "NOT LIKE", value: "%draft%" };
  ...
  describe("built-in fields", () => {
    test("id field", () => {
      const filter: FilterType<TestData> = { field: "id", op: "=", value: "abc123" };
      expect(buildFilter(filter)).toBe("id = 'abc123'");
    });

    test("text field", () => {
      const filter: FilterType<TestData> = { field: "text", op: "LIKE", value: "%hello%" };
  ...
  describe("logical combinators", () => {
    test("AND", () => {
      const filter: FilterType<TestData> = {
        AND: [
