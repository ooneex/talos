...
use cli::commands::add::{AddArgs, restore_files, snapshot_files, split_deps};
...
#[test]
fn add_parses_all_flags_and_deps() {
    ...
    assert_eq!(cli.args.deps, "lodash,zod");
    assert!(cli.args.dev);
    assert!(cli.args.optional);
    assert!(cli.args.peer);
    assert!(cli.args.exact);
...
#[test]
fn add_requires_deps() {
    assert!(TestCli::try_parse_from(["talos"]).is_err());
}

#[test]
fn add_defaults_are_empty() {
    let cli =
        TestCli::try_parse_from(["talos", "--deps", "lodash"]).expect("--deps alone is valid");
...
fn split_deps_trims_and_drops_empty_entries() {
    assert_eq!(
        split_deps(" lodash ,zod,, react "),
        vec!["lodash", "zod", "react"]
    );
}

#[test]
fn split_deps_is_empty_for_blank_input() {
    assert!(split_deps("").is_empty());
    assert!(split_deps(" , , ").is_empty());
}
...
fn restore_files_removes_files_that_did_not_exist_before() {
    ...
    // A resolve that created a lockfile that wasn't there before the
    // snapshot was taken (the first `bun add` in a fresh workspace).
