...
describe("runModuleScripts", () => {
  ...
  test("should forward the --no-cache flag when noCache is set", async () => {
    const moduleDir = join(testDir, "modules", "auth");
    await Bun.write(join(moduleDir, "package.json"), JSON.stringify({ name: "@acme/auth" }));
    await Bun.write(join(moduleDir, "bin", "migration", "up.ts"), "// migration");
    process.chdir(testDir);

    await runModuleScripts(logger, { ...MIGRATIONS, noCache: true });

    expect(spawnCalls[0]?.cmd).toEqual(["bun", "run", join(moduleDir, "bin", "migration", "up.ts"), "--no-cache"]);
  });

  test("should not forward the --no-cache flag when noCache is omitted", async () => {
    const moduleDir = join(testDir, "modules", "auth");
    await Bun.write(join(moduleDir, "package.json"), JSON.stringify({ name: "@acme/auth" }));
    await Bun.write(join(moduleDir, "bin", "migration", "up.ts"), "// migration");
    process.chdir(testDir);
