...
#[test]
fn database_create_parses_cloudflare_type() {
    let cli = TestCli::try_parse_from(["talos", "--type", "cloudflare"])
        .expect("cloudflare should be a valid database type");

    assert_eq!(cli.args.r#type, Some(DatabaseType::Cloudflare));
}
