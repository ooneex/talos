...
const testHome = mkdtempSync(join(nodeOs.tmpdir(), "talos-npm-credentials-"));
...
const { NpmCredentialsCreateCommand } = await import("@/commands/NpmCredentialsCreateCommand");

describe("NpmCredentialsCreateCommand", () => {
  let command: InstanceType<typeof NpmCredentialsCreateCommand>;
  const credentialsPath = join(testHome, ".talos", "credentials", "npm.yml");

  beforeEach(() => {
    command = new NpmCredentialsCreateCommand();
  ...
  describe("Command Metadata", () => {
    test("should return correct command name", () => {
      expect(command.getName()).toBe("npm:credentials:create");
    });

    test("should return correct description", () => {
      expect(command.getDescription()).toBe("Save an npm Granular Access Token under the user config");
    });
  });

  describe("run()", () => {
    test("should write the token to npm.yml under the user config", async () => {
      await command.run({ token: "npm_testtoken123", silent: true });
      ...
      expect(content).toContain("token: npm_testtoken123");
    });
    ...
    test("should print the token creation URL when not silent", async () => {
      await command.run({ token: "npm_testtoken123" });

      expect(infoCalls.some((message) => message.includes("granular-access-tokens"))).toBe(true);
    });
