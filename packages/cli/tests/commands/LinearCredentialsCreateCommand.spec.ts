...
const testHome = mkdtempSync(join(nodeOs.tmpdir(), "talos-linear-credentials-"));
...
const { LinearCredentialsCreateCommand } = await import("@/commands/LinearCredentialsCreateCommand");

describe("LinearCredentialsCreateCommand", () => {
  let command: InstanceType<typeof LinearCredentialsCreateCommand>;
  const credentialsPath = join(testHome, ".talos", "credentials", "linear.yml");

  beforeEach(() => {
    command = new LinearCredentialsCreateCommand();
  ...
  describe("Command Metadata", () => {
    test("should return correct command name", () => {
      expect(command.getName()).toBe("linear:credentials:create");
    });

    test("should return correct description", () => {
      expect(command.getDescription()).toBe("Save a Linear Personal API key under the user config");
    });
  });

  describe("run()", () => {
    test("should write the token to linear.yml under the user config", async () => {
      await command.run({ token: "lin_api_testtoken123", silent: true });
      ...
      expect(content).toContain("token: lin_api_testtoken123");
    });
    ...
    test("should print the token creation URL when not silent", async () => {
      await command.run({ token: "lin_api_testtoken123" });

      expect(infoCalls.some((message) => message.includes("linear.app/settings/api"))).toBe(true);
    });
