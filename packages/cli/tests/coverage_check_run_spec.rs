//! Runs `coverage:check` over a scratch workspace of real bun suites.
//!
...
use cli::commands::coverage_check::{RunStatus, audit};
...
fn the_report_draws_a_row_per_module_and_names_the_least_covered_files() {
    ...
    let output = talos(&root, &["coverage:check", "--no-cache", "--threshold=90"]);
...
fn logs_prints_the_output_of_the_suite_that_failed() {
    ...
    let quiet = text(&talos(
        &root,
        &["coverage:check", "--no-cache", "--modules=broken"],
    ));
    let loud = text(&talos(
        &root,
        &["coverage:check", "--no-cache", "--logs", "--modules=broken"],
    ));
...
fn strict_turns_a_thin_module_into_a_failing_run() {
    ...
    let lenient = talos(&root, &["coverage:check", "--no-cache", "--modules=thin"]);
    let strict = talos(
        &root,
        &["coverage:check", "--no-cache", "--modules=thin", "--strict"],
    );
...
fn the_second_run_is_served_from_the_cache() {
    ...
    talos(&root, &["coverage:check", "--modules=covered"]);
    let warm = text(&talos(&root, &["coverage:check", "--modules=covered"]));
...
fn editing_a_source_file_retires_the_cached_measurement() {
    let (_dir, root) = workspace();
    talos(&root, &["coverage:check", "--modules=covered"]);
    assert!(text(&talos(&root, &["coverage:check", "--modules=covered"])).contains("cached"));
    ...
    let after = text(&talos(&root, &["coverage:check", "--modules=covered"]));
...
fn issues_writes_one_yaml_per_module_that_misses_the_threshold() {
    ...
    let output = talos(
        &root,
        &["coverage:check", "--no-cache", "--issues", "--threshold=90"],
    );
    ...
    assert!(
        thin_issue.contains("talos coverage:check --modules=thin"),
