use clap::Parser;
use cli::commands::service_create::{ServiceCreateArgs, run};
use std::sync::Mutex;
...
#[test]
fn service_create_parses_all_flags() {
...
#[test]
fn service_create_defaults_are_empty() {
...
#[test]
fn service_create_rejects_unknown_flag() {
    assert!(TestCli::try_parse_from(["talos", "--definitely-not-a-flag"]).is_err());
}

#[test]
fn service_create_scaffolds_the_service_files() {
    let _guard = ENV_GUARD.lock().unwrap_or_else(|error| error.into_inner());
    let root = tempfile::tempdir().expect("tempdir");
    let templates = tempfile::tempdir().expect("tempdir");
    std::fs::write(
        templates.path().join("service.txt"),
        "export class {{NAME}}Service {}\n",
    )
    .expect("template");
    std::fs::write(
        templates.path().join("service.test.txt"),
        "// {{NAME}} in {{MODULE}}\n",
    )
    .expect("template");
    ...
    assert!(
        root.path()
            .join("modules/shared/src/services/BillingService.ts")
            .is_file()
    );
    assert!(
        root.path()
            .join("modules/shared/tests/services/BillingService.spec.ts")
            .is_file()
    );
}

#[test]
fn service_create_returns_when_templates_cannot_be_resolved_or_loaded() {
    let _guard = ENV_GUARD.lock().unwrap_or_else(|error| error.into_inner());
    let root = tempfile::tempdir().expect("tempdir");
    let previous_dir = std::env::current_dir().expect("cwd");
    ...
    let templates = tempfile::tempdir().expect("tempdir");
    std::fs::write(templates.path().join("service.txt"), "service").expect("template");
    unsafe {
        std::env::set_var(cli::utils::TEMPLATES_DIR_ENV, templates.path());
    }
    run(&ServiceCreateArgs {
    ...
    std::fs::write(templates.path().join("service.test.txt"), "spec").expect("template");
    std::fs::remove_file(templates.path().join("service.txt")).expect("remove");
    run(&ServiceCreateArgs {
        no_cache: false,
        name: Some("Billing".to_string()),
        module: Some("shared".to_string()),
