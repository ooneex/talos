...
Generate a controller class, route type, and test file, then complete the implementation. Follow the shared workflow in the `talos-scaffold` skill (run-from-root, `--name`/`--module` inference, module registration, lint/format, and coding conventions); this skill covers only the controller-specific parts.
...
- `--name` — the controller class name, taken from the resource/action (e.g., "a controller to list books" → `BookList`). Pass it in any casing; the CLI normalizes to PascalCase and appends the `Controller` suffix automatically, so omit the suffix.
...
- `--route-name` — dot notation `<resource>.<action>` derived from the request (e.g., "list books" → `book.list`, "create a user" → `user.create`).
- `--route-path` — the URL path, inferred from the resource as a kebab-case plural (e.g., books → `/books`); use what the user states if they give an explicit path.
- `--route-method` — HTTP method mapped from the verb (HTTP only): list/get/show → `get`, create → `post`, update/replace → `put` (partial → `patch`), delete/remove → `delete`.

Also generates `src/types/routes/<route.name>.ts` — delete that file after step 3 (keep the type inside the controller file).
...
**Keep the route type inside the controller file**, not in a separate type file. Delete the generated `src/types/routes/<route.name>.ts`.

**Include only what the route actually needs:**

- `params` — only for routes with URL path segments (e.g., `/users/:id`)
- `payload` — only for `post`, `put`, `patch` methods
- `queries` — only for list/search endpoints with query string filtering
- `response` — always include

```typescript
// GET /users (list) — queries, response
type <TypeName>RouteType = {
  queries: { },
  response: { },
};

// POST /users (create) — payload, response
type <TypeName>RouteType = {
  payload: { },
  response: { },
};

// GET /users/:id (detail) — params, response
type <TypeName>RouteType = {
  params: { },
  response: { },
};

// PUT /users/:id (update) — params, payload, response
type <TypeName>RouteType = {
  params: { },
  payload: { },
  response: { },
...
`roles` is an array of plain role strings (e.g. `["ROLE_USER"]`). There is **no `ERole` enum** — do not import anything from `@talosjs/role` in a controller. Access is hierarchical and graph-based: granting a role also grants every role it inherits (its ancestors, directly or transitively). Sibling roles on different branches do **not** satisfy each other.

The available roles are defined in `modules/shared/src/roles.yml`. **Always read that file** to use the project's actual roles, since each project can customize them. The default hierarchy (ancestor → descendant) is:
...
Read and replace `modules/<module>/tests/controllers/<Name>Controller.spec.ts`:

**Coverage:** class identity (`name.endsWith("Controller")`, is constructor), `index` exists and returns `Promise`, calls `context.response.json` (mock). Add response shape tests after injecting a mock service. Instance isolation.
...
**Only for `post`, `put`, `patch`, `delete` routes — skip for `get`.**
...
A controller's route is part of the SDK surface, so whenever you **create or update** a controller, refresh any SDK that exposes it via the `sdk-create` skill. An SDK module is any module whose `<name>.yml` has `type: "sdk"`; its `target` field records the module it was generated from (`app`, or a specific microservice).
...
Then complete the new or updated `api` method for this controller as described in the `sdk-create` skill. Skip this step when no SDK module targets the controller's module.

## Usage Examples

`Assert` is an alias for ArkType's `type` function. Use ArkType string syntax inside it — there is no fluent API like `Assert.string()` or `Assert.string().optional()`.

- Primitive: `"string"`, `"number"`, `"boolean"`
- Optional field: `"string?"`, `"number?"`
- Array: `"string[]"`, `"string[]?"`
- Union: `"string | number"`, `"File | Blob"`
- Format: `"string.email"`, `"number.integer"`
- Range: `"1 <= string <= 100"`, `"1 <= number.integer <= 65535"`
- Regex: `/^[a-z]+$/`
- Enum: `'"admin" | "user" | "guest"'`

For `params`, each URL segment gets its own `Assert("...")` call. For `queries`, `payload`, and `response`, pass an object to `Assert({...})`.
...
import type { ContextType } from "@talosjs/controller";
import { Route } from "@talosjs/routing";
import { Assert, AssertId } from "@talosjs/validation";
...
For nested params like `/users/:userId/bills/:billId` — one key per segment:

```typescript
type UserBillDetailRouteType = {
  params: { userId: string; billId: string };
...
export class UserCreateController {
  ...
  public async index(context: ContextType<UserCreateRouteType>) {
    ...
    // Never publish or return raw credentials. Hash the password in the service
    // before persistence, and publish only non-sensitive fields on the event.
...
### HTTP — PUT update (params + payload + response)
...
@Route.put("/users/:id", {
  ...
  params: { id: new AssertId() },
  payload: Assert({ name: "string?", email: "string.email?" }),
  response: Assert({ id: "string", name: "string", email: "string" }),
  roles: ["ROLE_ADMIN"],
})
export class UserUpdateController {
  constructor(private readonly userUpdatedEvent: UserUpdatedEvent) {}

  public async index(context: ContextType<UserUpdateRouteType>) {
    const { id } = context.request.params();
    const data = await context.request.payload();
    await this.userUpdatedEvent.publish({ id, ...data });
    return context.response.json({ id, name: data.name ?? "", email: data.email ?? "" });
  }
}
```
...
- `channel.send(response)` — send to this client only
- `channel.publish(response)` — broadcast to all channel subscribers
- `channel.subscribe()` / `channel.unsubscribe()` / `channel.isSubscribed()` — manage pub/sub
- `channel.close(code?, reason?)` — close the WebSocket connection
- `channel.ws` — raw `ServerWebSocket` instance
...
export class ChatMessageController {
  public async index(context: ContextType<ChatMessageRouteType>) {
    ...
    // Broadcast to all room subscribers (includes sender)
    await context.channel.publish(reply);
  }
}
```
