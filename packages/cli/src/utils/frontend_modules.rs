...
/// The ports the workspace's modules already claim.
///
...
pub fn rewrite_playwright_port(config_path: &Path, port: u16) {
    ...
    let Ok(port_re) = Regex::new(r"127\.0\.0\.1:(\d+)|--port (\d+)") else {
        return;
    };
    let rewritten = port_re
...
mod tests {
    ...
    #[test]
    fn sets_the_target_a_module_documents() {
        let yml = "type: \"swagger\"\ndesign: \"design\"\ntarget: \"app\"\n";
        assert!(with_target_field(yml, Some("gateway")).contains("target: \"gateway\""));
    }
    ...
    fn picks_the_first_port_nobody_claimed() {
        ...
        assert_eq!(find_free_port(3032, &used), 3034);
    }

    #[test]
    fn keeps_the_preferred_port_when_it_is_free() {
        assert_eq!(find_free_port(3032, &BTreeSet::new()), 3032);
    }
    ...
    fn ignores_the_port_of_the_module_being_generated() {
        ...
        assert_eq!(find_free_port(3032, &used), 3032);
    }
