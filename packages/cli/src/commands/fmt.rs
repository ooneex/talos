...
pub fn execute(args: &FmtArgs) -> bool {
    ...
    let mut group = build_group(&sorted, &included_keys, "fmt");
    if group.is_empty() {
        println!(
            "{}{}",
            style("↷ ").yellow(),
            style("Skipped fmt (no such script)").dim()
        );
        return true;
    }
    ...
    let loader = Loader::start(vec![LoaderGroup::new("Fmt", group.len())]);
