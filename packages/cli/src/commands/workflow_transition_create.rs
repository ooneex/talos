...
pub fn run(args: &WorkflowTransitionCreateArgs) {
    ...
    let Some(template) = read_template(&templates_dir, "workflow-transition.txt") else {
        return;
    };
    let Some(test_template) = read_template(&templates_dir, "workflow-transition.test.txt") else {
        return;
    };
    scaffold_resource(
        &ScaffoldConfig {
