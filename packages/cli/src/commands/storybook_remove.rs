...
pub fn run(args: &StorybookRemoveArgs) {
    let name = match args.name.clone() {
        Some(name) => name,
        None => match ask_input("Enter storybook name to remove") {
            Some(name) => name,
            None => return,
        },
    };
    ...
    if !module_dir.join("package.json").exists() {
        if !silent {
            crate::utils::error(format!("Storybook \"{kebab_name}\" does not exist"));
        }
        return;
    }

    let yml_path = module_dir.join(format!("{kebab_name}.yml"));
    let is_storybook_module = yml_path.exists()
        && std::fs::read_to_string(&yml_path)
            .map(|content| content.contains("type: \"storybook\""))
            .unwrap_or(false);
    if !is_storybook_module {
        if !silent {
            crate::utils::error(format!("Module \"{kebab_name}\" is not a storybook module"));
        }
        return;
    }
    ...
    if !silent
        && !ask_confirm(
            &format!("Are you sure you want to remove the \"{kebab_name}\" storybook?"),
            false,
        )
    {
        return;
