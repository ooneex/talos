...
use crate::utils::{
    add_module_alias_if_present, ask_input, clone_frontend_template, collect_used_ports,
    current_dir, ensure_design_module, finalize_module_yml, find_free_port,
...
pub fn run(args: &SwaggerCreateArgs) {
    ...
    if let Err(error) = clone_frontend_template("swagger", &module_dir, args.no_cache) {
    ...
    let port = find_free_port(DEFAULT_PORT, &collect_used_ports(&modules_dir, &kebab_name));
    ...
    rewrite_module_imports(&src_dir, "swagger", &kebab_name);
    rewrite_vite_alias(&module_dir.join("vite.config.ts"), design_kebab.as_deref());

    let written = write_documentation(&module_dir, &routes, &prefix);

    if !install_frontend_dependencies(&cwd, "swagger", &deps, &dev_deps, silent) {
        return;
    }
