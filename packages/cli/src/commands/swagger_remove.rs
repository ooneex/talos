...
pub fn run(args: &SwaggerRemoveArgs) {
    let name = match args.name.clone() {
        Some(name) => name,
        None => match ask_input("Enter swagger name to remove") {
            Some(name) => name,
            None => return,
        },
    };
    ...
    if !module_dir.join("package.json").exists() {
        if !silent {
            crate::utils::error(format!("Swagger \"{kebab_name}\" does not exist"));
        }
        return;
    }

    let yml_path = module_dir.join(format!("{kebab_name}.yml"));
    let is_swagger_module = yml_path.exists()
        && std::fs::read_to_string(&yml_path)
            .map(|content| content.contains("type: \"swagger\""))
            .unwrap_or(false);
    if !is_swagger_module {
        if !silent {
            crate::utils::error(format!("Module \"{kebab_name}\" is not a swagger module"));
        }
        return;
    }
    ...
    if !silent
        && !ask_confirm(
            &format!("Are you sure you want to remove the \"{kebab_name}\" swagger?"),
            false,
        )
    {
        return;
