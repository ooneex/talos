...
@decorator.command()
export class SpaRemoveCommand<T extends CommandOptionsType = CommandOptionsType> implements ICommand<T> {
  public getName(): string {
    return "spa:remove";
  }

  public getDescription(): string {
    return "Remove an existing spa module";
  }
  ...
  public async run(options: T): Promise<void> {
    ...
    if (!name) {
      name = await askName({ message: "Enter spa name to remove" });
    }
    ...
    if (!moduleDirExists) {
      if (!silent) {
        const logger = new TerminalLogger();
        logger.error(`Spa "${kebabName}" does not exist`, undefined, LOG_OPTIONS);
      }
      return;
    }

    // Only spa modules can be removed with this command
    const ymlPath = join(moduleDir, `${kebabName}.yml`);
    const isSpaModule = (await Bun.file(ymlPath).exists()) && (await Bun.file(ymlPath).text()).includes('type: "spa"');

    if (!isSpaModule) {
      if (!silent) {
        const logger = new TerminalLogger();
        logger.error(`Module "${kebabName}" is not a spa module`, undefined, LOG_OPTIONS);
      }
      return;
    }
    ...
    if (!silent) {
      const confirmed = await askConfirm({
        message: `Are you sure you want to remove the "${kebabName}" spa?`,
        initial: false,
      });
    ...
    // A spa module is not registered into AppModule or SharedModule, but clean up any leftover references
