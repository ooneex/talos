...
pub type AdminCreateArgs = DesignWithTargetCreateArgs;
...
pub fn run(args: &AdminCreateArgs) {
    ...
    let name = args
        .name
        .clone()
        .or_else(|| ask_input("Enter admin name"))
        .unwrap_or_default();
    if name.is_empty() {
        return;
    }
    ...
    if let Err(error) = clone_frontend_template("admin", &module_dir, args.no_cache) {
    ...
    rewrite_module_imports(&src_dir, "admin", &kebab_name);
    rewrite_vite_alias(&module_dir.join("vite.config.ts"), design_kebab.as_deref());
    ensure_shared_placeholder(&src_dir);

    if !install_frontend_dependencies(&cwd, "admin", &deps, &dev_deps, silent) {
        return;
    }
