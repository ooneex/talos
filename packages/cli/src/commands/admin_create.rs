...
fn install_root_dependencies(
    ...
    if !dev_deps.is_empty()
        && !run_spinner_step(
            false,
            "Installing admin dev dependencies",
            Command::new("bun")
                .args(["add", "-D"])
                .args(dev_deps)
                .current_dir(cwd),
...
pub fn run(args: &AdminCreateArgs) {
    let cwd = args
        .cwd
        .clone()
    ...
    let name = match args.name.clone() {
        Some(name) => name,
        None => match ask_input("Enter admin name") {
            Some(name) => name,
            None => return,
        },
    };
    ...
    let template_dir = repo_dir.join("modules").join("admin");
    ...
    if let Err(error) = copy_dir(&template_dir, &module_dir, &options) {
        crate::utils::error(format!("Failed to copy admin template: {error}"));
        return;
    }

    let template_yml = module_dir.join("admin.yml");
    ...
    visit_files_recursive(&src_dir, &mut |file_path| {
        if let Ok(content) = fs::read_to_string(file_path) {
            let rewritten = regex::Regex::new(r#"from \"@module/admin(["/])"#)
                .ok()
                .map(|re| {
