...
oo project:check --only=runtime,data             # Only the named categories
oo project:check --skip=frontend                 # Skip a whole category
...
oo project:check --no-cache                      # Ignore var/cache/project and re-run everything
oo project:check --strict                       # Exit 1 when a check only reports warnings
oo project:check --json                         # Machine-readable report for CI
```

`project:check` is the whole-project gate. It runs fifty-nine checks — `workspace` (`monorepo:run --commands=install,build,fmt,lint,test`), `structure` (module manifests and types, `package.json` names, root `workspaces` globs and `tsconfig.json` aliases), `folders` (every directory of every module against the layout its `type:` allows — the backend artifact folders, a design system's asset kinds, the `bootstrap`/`routes`/`features`/`shared` halves of a spa or admin, the storybook's `story/` engine, an SDK's flat `src/` — with `tests/` held to the layout it mirrors; a folder the layout does not name is an error, so a hand-made `src/lib/` or `shared/helpers/` is reported once, with what may sit at that level instead), `tsconfig` (every module extending the root config, strictness flags it must not relax, `node_modules`/`dist` exclusions), `lockfile` (a single package manager, no lockfile nested in a member, every declared dependency present, manifests newer than the lock), `conventions` (DI decorators against class-name suffixes, direct `process.env` reads, exported `Type`/`I` naming, non-null assertions), `imports` (local specifiers that resolve to nothing, module and file cycles, and the dependency rule — controller → service → repository → entity, never the other way), `boundaries` (which module may import which, by runtime: a browser module bundling a server one and a server module importing a browser one both fail, while a design module depending on an application only warns), `restricted` (`node:`/`bun:` runtimes imported into a browser module, packages the @talosjs ecosystem already answers, and the ORM imported outside the repository layer), `container` (every `@inject`ed class the workspace declares carrying a decorator that binds it — the token that only fails when the container is built), `registration` (every `@Route`, `@Entity` and `@decorator.middleware`/`cron`/`event` class listed in its `<Name>Module.ts`, and nothing listed that no longer exists), `middlewares` (a middleware whose `handler` never returns the context — the chain silently stops there — plus one with no handler at all, one nothing references, and two sharing a class name so the order they run in is undefined), `routes` (endpoint and route-name collisions, missing `name`/`version`, paths that do not start with `/`, routes with no `roles`), `openapi` (a swagger module's published `openapi.json`/`.yml` against the controllers of its `target:` module, comparing operations with the version prefix and `:id`/`{id}` normalised away — an endpoint served but not published, or published but served by nothing), `pagination` (a `GET` route returning a collection — an arktype array, a `FilterResultType`, or a route named `*.list`/`*.index`/`*.all` — that declares neither `page` nor `limit` in its `queries`), `validation` (each controller's `RouteType` against the `Assert` schema beside it — a field typed but never validated fails, one validated but untyped warns), `roles` (route guards against the names `roles.yml` declares, plus the hierarchy's own consistency: unknown parents, self-inheritance, loops), `permissions` (a `@decorator.permission()` class still carrying the generator's placeholder: a `check()` that returns `true` unconditionally, an `allow()` that grants no ability, a `setUserPermissions()` that ignores the user, or a class nothing reads), `entities` (every `@Entity` table and `@Column` name mentioned by a migration, and one table per entity), `indexes` (every `@JoinColumn` foreign key, `unique: true` column and `*_id`/`*Id` column against the `CREATE INDEX`/`createIndex`/`UNIQUE` statements the module's migrations actually contain), `repositories` (a repository built on an entity class that no longer exists, one nothing injects, a hard `delete` on a soft-deletable entity, and any service or controller reaching past the repository layer with `database.open`, `createQueryBuilder` or `getRepository`), `transactions` (a method outside the repository layer performing two or more writes — `save`, `insert`, `update`, `delete`, `upsert`, `increment` — with no `transaction(`, `queryRunner` or `manager.transaction` around them), `sql` (a value interpolated into a raw query in a migration, repository, seed or database file), `async` (an `await` inside a loop, a `forEach` with an async callback, a `map` building promises nobody awaits), `exceptions` (`throw new Error`/`TypeError` and thrown string or object literals, which carry no code and come back as a 500, plus empty `catch` blocks and `*Exception` classes that extend nothing), `logging` (`console.*` calls, which have no level and no destination, and any log line whose arguments name a `password`, `token`, `secret`, `apiKey`, `authorization`, card or SSN field — read by balancing parentheses so a call spread over several lines is still read whole), `complexity` (500-line file, 80-line function, 5-parameter and 5-level nesting budgets, with the nesting and body budgets off for JSX), `orphans` (files nothing imports and exports nothing consumes, skipping entry points and the library modules — `packages/`, `design`, `sdk` — whose exports are the product), `events` (two `@decorator.event()` classes claiming one channel so both handlers fire, a class declaring no channel, a handler that does nothing, and an event nothing publishes to), `queues` (two queues sharing a name and splitting the same job stream, a queue with no `onFailed` so an exhausted job disappears silently, and one nothing ever enqueues to), `crons` (a `getTime()` sentence the crontab conversion rejects — a bad prefix, unit or number — or one it accepts and then betrays, such as `every 90 minutes` becoming `*/90` in a field that only holds 59, or `every 3 years` running yearly regardless; plus empty handlers and jobs stacked on the same tick), `workflows` (a `@decorator.transition()` class in no workflow's `getTransitions()` so it never runs, a workflow listing a transition that no class declares, a workflow with no transitions, a duplicate workflow name, and a transition that does work with an empty `rollback`), `mailers` (a `@decorator.mailer()` class with no `<Name>MailerTemplate` to render, a template no sender renders, a mailer with no `send`, and one nothing sends), `flags` (two feature flags claiming one key, a flag with no key or no description, and a flag nothing reads — by class on the backend or by key string on the front end), `env` (each `.env.example.yml` against its local `.env.yml`), `dependencies` (one range per dependency, unpinned ranges, undeclared imports, unused packages), `docker` (compose services, unpinned images, missing `image`/`build`, clashing host ports), `migrations` (unique ordered timestamps, `up`/`down` presence, seed YAML validity), `accessibility` (Biome's `a11y` rules over every UI module's `src/`: design, spa, admin, storybook), `contrast` (the real WCAG ratio of every `--X-foreground` token against the `--X` surface it sits on, resolved through `var()` aliases and computed from `oklch()` per theme file — under 3:1 fails, under 4.5:1 warns), `tokens` (a colour written by hand in a component — a hex literal, `rgb()`, `oklch()`, `color-mix()` — which fails in a module that consumes the design system and warns in the one that defines it, plus Tailwind arbitrary values like `text-[13px]` and inline `style={{…}}`, both exempt inside the design module and inside `styles/`, `icons/`, `fonts/` and `themes/`), `assets` (files under `public/` and `src/` that nothing references — bar the ones the platform loads by name, `favicon`, `robots`, `manifest` and their kin — images over 300 KB, fonts over 200 KB, media over 2 MB, and any `<img>` with no `width` and `height`, which reflows the page when it loads), `translations` (the `en` fallback, locale parity, `{{ placeholder }}` drift, keys looked up that no dictionary defines and keys nothing looks up), `stories` (a story for every design-system component, story `meta`, and stories importing nothing from the module they document), `router` (every file under `src/routes/` against the route id its location implies — the flat `posts.$id.tsx` and the nested `posts/$id.tsx` addressing the same route, `index` addressing its parent, `(group)` folders adding no segment — plus a file with no `createFileRoute`, a route declared twice, a route the generated `routeTree.gen.ts` has never seen, and the missing `errorComponent`/`pendingComponent`/`notFoundComponent` boundaries), `queries` (a TanStack Query `queryKey` written inline instead of read from the feature's key factory, one key root shared by two different features, and a `useMutation` that never invalidates, sets or resets the cache — the screen that keeps showing what the user just changed), `sdk` (each generated client against the controllers of its `target:` module — routes with no method, methods for routes that are gone, endpoints that moved), `tests` (a mirrored `.spec.ts` for every source file that holds behaviour), `e2e-coverage` (a module that serves an application or routes and has no spec at all, plus specs with no `playwright.config.ts` or no `e2e` script to run them), `docs` (relative markdown links that no longer resolve), `bundle` (5 MB per build, 1 MB per chunk and 500 KB per image budgets, shipped source maps, `dist/` older than `src/`), `security` (the OSV.dev dependency audit plus the LLM configuration audit of every assistant's agents, skills, rules and MCP files), `secrets` (credential formats in the working tree plus `.env`/`.pem` files tracked by git), `git` (build output and dependency trees in the index, large blobs, `.gitignore` gaps), `issues` (the `issue:check` conventions), `todos` (a `TODO(OON-123456)`/`FIXME(…)` marker naming an issue the repository does not hold, or one whose issue is already `Done` or `Canceled` — the bare-marker case belongs to `hygiene`), `branches` (the issue files against git itself: an open issue whose branch exists nowhere, a finished one whose branch was never deleted, a branch that traces back to no issue), `commits` (conventional-commit rules over the unpushed commits) and `hygiene` (conflict markers, focused/skipped tests, bare `TODO`s) — plus two opt-in checks: the `e2e` suite with `--e2e`, and `outdated` with `--outdated`, which compares every declared floor against npm, crates.io and PyPI and reports only the majors behind. It prints a status line per check, a detail block per non-passing check, and one verdict line. Each check reuses the code of its dedicated command, so it can never disagree with `monorepo:check`, `security:check` or `issue:check`. Check names accept aliases (`a11y`, `audit`, `deps`, `i18n`, `layout`, `naming`, `compose`, `seeds`, `specs`, `markdown`, `gitignore`, `commit`, `monorepo`, `cycles`, `layers`, `wiring`, `endpoints`, `schema`, `dead-code`, `lock`, `dist`, `updates`, `di`, `coupling`, `banned`, `assert`, `permissions`, `await`, `wcag`, `branch`), a check with nothing to inspect is reported as **skipped** rather than passed, generated sources (`*.gen.ts`, `@generated` banners) are exempt from the convention rules, `import type` edges are excluded from cycle detection because the compiler erases them, and violations of a11y rules the project disabled in `biome.jsonc` are reported separately as a non-failing "not enforced" note. Every check also belongs to one of nine **categories** — `foundation`, `architecture`, `api`, `data`, `runtime`, `frontend`, `quality`, `supply-chain`, `process` — which the report groups the summary table under and which `--only` and `--skip` accept in place of the checks they hold, so `--only=runtime,data` and `--skip=frontend` do what naming sixteen checks would. No category is spelled the way a check is: `workspace` and `security` keep meaning the single check each has always meant. The checks **run concurrently** — every one but the workspace gate and the end-to-end suite only reads files, so there is nothing to serialise them for; those two still run alone and in that order, the first because its install is what puts the tools the others shell out to on disk, the last because it boots the application. Results are **cached per module and package** under `var/cache/project/<check>.json`. An entry records the fingerprint of every workspace member the check could have read, plus one for everything outside `modules/` and `packages/`, and is replayed only when all of them still match — so a run on an untouched tree does no work at all, an edit to a design system leaves `entities` and `indexes` cached, and a new migration leaves `tokens` and `router` cached. File hashes are memoised by size and modification time in `var/cache/project/filehashes.json`, which turns the fingerprint of an unchanged member into a directory walk. Seven checks are never cached, because their answer is not a function of the tree: `workspace` and `e2e` *do* something rather than report on something (and already cache themselves at the task level), `security` and `outdated` ask the network, and `git`, `commits` and `branches` read the repository rather than the files in it. `--no-cache` bypasses reading and writing; `--json` and `--strict` never invalidate anything, because they change the report rather than a finding. Exit code is `1` on any failure (or any warning with `--strict`).

`folders` is the strictest of them and the one worth stating on its own: **only the folders a layout names may exist.** A backend module holds `src/`, `tests/`, `e2e/` and `issues/`, and its `src/` holds only the artifact folders the generators create (`ai/` with `chats`, `middlewares`, `tools`; `workflows/` with `transitions`; the rest flat). A design module's `src/` holds `components`, `fonts`, `hooks`, `icons`, `styles`, `translations`, `utils`, with icons at `icons/<fill|outline>/<category>/<sm|md|lg>` and stylesheets allowing only a `themes/` subfolder. A spa or admin module adds `public/` and holds `bootstrap` (files only), `routes` (URL segments, free), `features/<feature>` and `shared`, both limited to the ten layers `assets`, `components`, `hooks`, `layouts`, `services`, `store`, `styles`, `translations`, `types`, `utils`. A storybook is a spa whose `shared/` also holds `story/` and whose features are folders of story files. An SDK's `src/` holds files and no folder. Below a leaf — inside `controllers/`, inside a design component, inside a feature's `components/` — grouping is free, because organising files of one kind is not inventing a kind. A package under `packages/` is only held to a layout when its manifest declares a `type:`; without one it is a plain library.
...
The checks added on top of the TypeScript ones follow the same rule. `complexity` measures `.rs` and `.py` alongside `.ts`/`.tsx`; `lockfile` knows `Cargo.lock`, `uv.lock`, `poetry.lock`, `Pipfile.lock` and `pdm.lock`, and only rejects a *nested* one when the root already locks the member; and `outdated` resolves npm ranges against registry.npmjs.org, Cargo requirements against crates.io and PEP 508 specifiers against PyPI. `imports`, `boundaries`, `restricted`, `container`, `registration`, `middlewares`, `routes`, `openapi`, `pagination`, `validation`, `permissions`, `entities`, `indexes`, `repositories`, `transactions`, `sql`, `async`, `exceptions`, `logging`, `orphans`, `events`, `queues`, `crons`, `workflows`, `mailers`, `flags`, `tokens`, `assets`, `stories`, `router`, `queries` and `sdk` read the TypeScript graph and are skipped where there is none, and `contrast` needs a `design` module's stylesheets.
...
The same command also runs an **LLM configuration audit** — instruction files are executable too, and reach the repository through the same untrusted supply chain as any dependency. It scans the agent, skill, rule, command and MCP files of every supported assistant (`.claude`, `.codex`, `.cursor`, `.gemini`, `.windsurf`, `.cline`/`.clinerules`, `.junie`, `.roo`/`.roomodes`, `.continue`, `.zed`/`.agents`/`.rules`, GitHub Copilot's `.github/copilot-instructions.md`/`agents`/`instructions`/`prompts`, plus the root `AGENTS.md`, `CLAUDE.md`, `GEMINI.md` and `.mcp.json`), at the workspace root and inside every module. Rules map to the OWASP LLM Top 10 and cover injected instructions overriding the system prompt, orders to hide actions from the user, invisible Unicode smuggling, data exfiltration, credential access, downloaded content piped into a shell, obfuscated payloads, destructive commands, approval/sandbox bypasses, remotely fetched instructions, unpinned MCP servers, hardcoded credentials and compromised links — a shortener, a punycode or non-ASCII homograph host, an `@` or credentials in the authority, a bare IP outside the private ranges, an anonymous paste/drop host, a `javascript:`/`data:text/html` target, and markdown whose visible label names a different domain than the URL it opens. Each finding reports the offending `file:line`, the evidence, the fix and an OWASP reference, and flows into `--issues` and `project:check` exactly like a dependency finding. Use `--skip-llm` to audit dependencies only.
