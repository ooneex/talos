...
`project:check` is the whole-project gate. It runs seventeen checks — `workspace` (`monorepo:run --commands=install,build,fmt,lint,test`), `structure` (module manifests and types, `package.json` names, root `workspaces` globs and `tsconfig.json` aliases), `conventions` (DI decorators against class-name suffixes, direct `process.env` reads, exported `Type`/`I` naming, non-null assertions), `env` (each `.env.example.yml` against its local `.env.yml`), `dependencies` (one range per dependency, unpinned ranges, undeclared imports, unused packages), `docker` (compose services, unpinned images, missing `image`/`build`, clashing host ports), `migrations` (unique ordered timestamps, `up`/`down` presence, seed YAML validity), `accessibility` (Biome's `a11y` rules over every UI module's `src/`: design, spa, admin, storybook), `translations` (the `en` fallback, locale parity and `{{ placeholder }}` drift in every dictionary), `tests` (a mirrored `.spec.ts` for every source file that holds behaviour), `docs` (relative markdown links that no longer resolve), `security` (the OSV.dev dependency audit), `secrets` (credential formats in the working tree plus `.env`/`.pem` files tracked by git), `git` (build output and dependency trees in the index, large blobs, `.gitignore` gaps), `issues` (the `issue:check` conventions), `commits` (conventional-commit rules over the unpushed commits) and `hygiene` (conflict markers, focused/skipped tests, bare `TODO`s) — plus the opt-in `e2e` suite with `--e2e`. It prints a status line per check, a detail block per non-passing check, and one verdict line. Each check reuses the code of its dedicated command, so it can never disagree with `monorepo:check`, `security:check` or `issue:check`. Check names accept aliases (`a11y`, `audit`, `deps`, `i18n`, `layout`, `naming`, `compose`, `seeds`, `specs`, `markdown`, `gitignore`, `commit`, `monorepo`), a check with nothing to inspect is reported as **skipped** rather than passed, generated sources (`*.gen.ts`, `@generated` banners) are exempt from the convention rules, and violations of a11y rules the project disabled in `biome.jsonc` are reported separately as a non-failing "not enforced" note. Exit code is `1` on any failure (or any warning with `--strict`).
