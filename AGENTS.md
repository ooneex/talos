...
The checks added on top of the TypeScript ones follow the same rule. `complexity` measures `.rs` and `.py` alongside `.ts`/`.tsx`; `lockfile` knows `Cargo.lock`, `uv.lock`, `poetry.lock`, `Pipfile.lock` and `pdm.lock`, and only rejects a *nested* one when the root already locks the member; and `outdated` resolves npm ranges against registry.npmjs.org, Cargo requirements against crates.io and PEP 508 specifiers against PyPI. `imports`, `registration`, `routes`, `entities`, `orphans`, `stories` and `sdk` read the TypeScript graph and are skipped where there is none.
